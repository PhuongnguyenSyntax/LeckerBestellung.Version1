...
) : Position(name, geburtsdatum, adresse, telefonNr, email, abteilung, gehalt, ueberstunde)
{
    ...
    // für Jede büberstunde bekommen Mitarbeiter 20€, Methoden zum Monaliche Bonus rechnen.
    var stundenLohn : Double = 20.0
    fun calculateMonatlicheBonus(employee: Position) {
        if (employee is Programmierer || employee is Buchhalter) {
            val ueberstunden = employee.ueberstunde
            println("Monatliche Bonus für Mitarbeite ${employee.name}: ${ueberstunden * stundenLohn}")
        }
    }
